...
handle_cast({log_can, TS, FrameID, DataLen, Data}, #st{cfg = Cfg} = S) ->
	...
	{ok, #i{buf = B, max = Max} = I} ->
	    io:format("exodemo_log(): Will log Frame ~p  Max ~p DataLen ~p Data ~p~n",
...
handle_info({timeout, _, {send, FrameID}}, #st{cfg = Cfg} = S) ->
	...
	{ok, #i{buf = B, int = Int} = I} ->
	    ...
	    {noreply, S#st{cfg = Cfg1}};
	error ->
	    io:format("exodemo_log(timeout): Nope ~p ~p~n", [FrameID, Cfg ]),
	    {noreply, S}
    end;
handle_info(_Msg, S) ->
    {noreply, S}.
	...
	[_|_] = List ->
	    ...
	    LogData = [{struct, [{ts, unix_time()},
				 ...
				 {'can-value',
				  ...
				  %% exodemo_lib:can_data_value(Len, binary:encode_unsigned(Data))
							    }]}
		       || {_, {TS, Len, Data}} <- List],
	...
	#conf_tree{tree = T} ->
		...
		lists:foldl(
		  fun({ID, Attrs}, Acc) ->
			  ...
			  io:format("exodemo_log(): Starting timer for Frame ~p Int ~p~n",
				    [ID, Int]),
